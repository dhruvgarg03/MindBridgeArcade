...
buttons = [
    {"name": "Tic Tac Toe", "rect": pygame.Rect((screen_width - button_width) // 2, 150, button_width, button_height), "action": None},
    ...
    {"name": "Maze Runner", "rect": pygame.Rect((screen_width - button_width) // 2, 150 + 2 * (button_height + button_spacing), button_width, button_height), "action": None},
    {"name": "Color Sort", "rect": pygame.Rect((screen_width - button_width) // 2, 150 + 3 * (button_height + button_spacing), button_width, button_height), "action": None},
    {"name": "Dots & Connect", "rect": pygame.Rect((screen_width - button_width) // 2, 150 + 4 * (button_height + button_spacing), button_width, button_height), "action": None},
    {"name": "Sudoku", "rect": pygame.Rect((screen_width - button_width) // 2, 150 + 5 * (button_height + button_spacing), button_width, button_height), "action": None}
]
            ...
            if event.type == pygame.QUIT:
                ...
                for btn in buttons:
                    if btn["rect"].collidepoint(event.pos):
                        if btn["name"] == "Tic Tac Toe":
                            from games.tic_tac_toe.main import run
                            run()
                        elif btn["name"] == "Chess":
                            from games.chess.main import run
                            run()
                        elif btn["name"] == "Maze Runner":
                            pygame.display.quit()
                            from games.maze_runner.main import run
                            run()
                            sys.exit()
                        elif btn["name"] == "Color Sort":
                            from games.color_sort.main import run
                            run()
                        elif btn["name"] == "Dots & Connect":
                            from games.dots_connect.main import run
                            run()
                        elif btn["name"] == "Sudoku":
                            from games.sudoku.main import run
                            run()
